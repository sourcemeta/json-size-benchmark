...
def decode(payload):
    return {
        ...
        'rules': [
            {x: y for x, y in payload.rules[0].__dict__.items() if y is not None},
            {x: y for x, y in payload.rules[1].__dict__.items() if y is not None},
            {x: y for x, y in payload.rules[2].__dict__.items() if y is not None},
            {x: y for x, y in payload.rules[3].__dict__.items() if y is not None},
            {x: y for x, y in payload.rules[4].__dict__.items() if y is not None}
        ]
    }
